...
.PHONY: start-dev dev dev-all dev-shell dev-topology dev-terminal package-topology package-vnc-viewer package-terminal macnat-module macnat-module-clean build test

start-dev: package-topology package-vnc-viewer package-terminal dev-shell

dev: start-dev

dev-all: package-topology package-vnc-viewer package-terminal
	...
	cleanup() {
		trap - EXIT INT TERM
		kill -- -$$shell_pid -$$topology_pid -$$shell_vite_pid >/dev/null 2>&1 || true
		wait $$shell_pid $$topology_pid $$shell_vite_pid >/dev/null 2>&1 || true
	}
	trap cleanup EXIT
	trap 'cleanup; exit 130' INT TERM
	wait -n $$shell_pid $$topology_pid $$shell_vite_pid
...
dev-topology: package-topology package-vnc-viewer package-terminal
	@echo "Topology app: http://$(TOPOLOGY_ADDR)"
	@setsid env GOCACHE="$(GOCACHE)" GOPROXY="$(GOPROXY)" $(GO) run . app run dist/apps/topology.foxapp --workspace "$(WORKSPACE)" --addr "$(TOPOLOGY_ADDR)" --uri "$(LIBVIRT_URI)" &
	topology_pid=$$!
	cleanup() {
...
build: package-topology package-vnc-viewer package-terminal
